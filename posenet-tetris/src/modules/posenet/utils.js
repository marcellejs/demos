...
import { getAdjacentKeyPoints, getBoundingBox, decodeSinglePose } from '@tensorflow-models/posenet';

const pointColor = 'aqua';
...
export function drawSkeleton(keypoints, minConfidence, ctx, scale = 1) {
  ...
  adjacentKeyPoints.forEach(someKeypoints => {
    drawSegment(
      toTuple(someKeypoints[0].position),
      toTuple(someKeypoints[1].position),
      pointColor,
      scale,
      ctx,
    );
...
export function drawKeypoints(keypoints, minConfidence, ctx, scale = 1) {
  for (let i = 0; i < keypoints.length; i += 1) {
    ...
    if (keypoint.score >= minConfidence) {
      ...
      drawPoint(ctx, y * scale, x * scale, 3, pointColor);
    }
  }
}
...
export function drawHeatMapValues(heatMapValues, outputStride, canvas) {
  const ctx = canvas.getContext('2d');
  const radius = 5;
  const scaledValues = heatMapValues.mul(scalar(outputStride, 'int32'));

  drawPoints(ctx, scaledValues, radius, pointColor);
}
...
export function drawOffsetVectors(heatMapValues, offsets, outputStride, scale = 1, ctx) {
  const offsetPoints = decodeSinglePose.getOffsetPoints(heatMapValues, outputStride, offsets);
  ...
  for (let i = 0; i < heatmapData.length; i += 2) {
    ...
    drawSegment([heatmapY, heatmapX], [offsetPointY, offsetPointX], pointColor, scale, ctx);
  }
}
...
export async function loadVideo(videoWidth, videoHeight) {
